                   ...
                   QString *error)
{
    ...
    options.comment = QStringLiteral("generated by pointcloudstitch transform-only regression");
...
int runRegression(const QStringList &arguments)
{
    ...
    const QCommandLineOption inputOption(QStringLiteral("input-dir"), QStringLiteral("PLY 目录"), QStringLiteral("path"));
    ...
    root.insert(QStringLiteral("ply_encoding"), QStringLiteral("binary_little_endian"));
    ...
    if (!outputDirectory.isEmpty()) {
        ...
        if(options.enabled){
            for(int index=0;index<result.diagnosticPreIcpFrames.size();++index){
                ...
                if(!writeBinaryPlyFile(prePath,result.diagnosticPreIcpFrames[index],&outputError)
                    ||!writeBinaryPlyFile(postPath,result.diagnosticPostIcpFrames.value(index),&outputError))return 3;
        ...
        if(allAccepted&&!result.points.isEmpty()){
            ...
            if (!writeBinaryPlyFile(mergedPath, result.points, &outputError)) return 3;
            ...
            if (!writeBinaryPlyFile(previewPath, preview, &outputError)) return 3;
