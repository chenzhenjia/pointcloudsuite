...
10. Point-to-Plane ICP 围绕重叠对应点质心求解旋转，并将 Hessian 投影到可观测特征子空间，抑制大平面的平面内退化滑移。
...
12. 累计修正超过 `10 mm / 1 deg` 时失败，不生成正式合并结果。
13. 沿修正后的拼接轴计算相邻点云真实投影重叠区，以重叠区中点建立 `±8 mm` 接缝带；仅在两侧均有点时融合互为最近邻点并用二维决策块处理未匹配点，否则完整保留两帧。
...
输出均为 binary little-endian PLY，正式结果没有一千万点限制。
...
位姿文件每行格式为 `A01 Start X... Y... Z... RX... RY... RZ... ; END X... Y... Z... RX... RY... RZ...`。报告包含全部相邻对的裁剪点数、投影重叠宽度、最近距离分位数、距离覆盖率、主水平面诊断和 ICP 结果。回归模式不写正式合并 PLY。
