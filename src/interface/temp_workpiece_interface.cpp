...
bool parseCreatedAt(const QJsonObject &root, TempScanningInfo *info, QString *error)
{
    const QJsonValue value = root.value(QStringLiteral("created_at"));
    if (value.isUndefined()) {
        info->warning = QStringLiteral("created_at missing");
        return true;
    }
    if (!value.isString()) {
        return fail(error, QStringLiteral("created_at must be an ISO 8601 string"));
    }
    const QString text = value.toString().trimmed();
    const QDateTime parsed = QDateTime::fromString(text, Qt::ISODateWithMs);
    if (text.isEmpty() || !parsed.isValid()) {
        return fail(error, QStringLiteral("created_at is not a valid ISO 8601 timestamp"));
    }
    info->createdAtIso8601 = text;
    return true;
}
...
bool buildAutomaticPlaneSelection(const QVector<pointcloud::Point3D> &points,
                                  const QVector<qsizetype> &sourceIndices,
                                  int minimumPlaneInliers,
                                  PlaneSelection *selection,
                                  QString *error)
{
    ...
    finiteIndices.reserve(points.size());
    for (int i = 0; i < points.size(); ++i) {
        if (finiteVector(toVector(points[i]))) finiteIndices.push_back(i);
    }
    if (finiteIndices.size() < 3) {
        return fail(error, QStringLiteral("automatic plane selection needs at least 3 finite points"));
    }

    // Use real points from a deterministic, bounded sample to score candidate
    // planes. The winning hypothesis is verified once against the full cloud
    // below; this avoids rescanning millions of points for every triangle.
    constexpr int kMaximumSamples = 4096;
    constexpr int kMaximumCandidates = 64;
    QVector<int> samples;
    samples.reserve(qMin(kMaximumSamples, finiteIndices.size()));
    if (finiteIndices.size() <= kMaximumSamples) {
        samples = finiteIndices;
    } else {
        for (int i = 0; i < kMaximumSamples; ++i) {
            const qsizetype position = (qsizetype(i) * (finiteIndices.size() - 1))
                / (kMaximumSamples - 1);
            samples.push_back(finiteIndices[int(position)]);
        }
    }

    QVector<int> candidates;
    candidates.reserve(qMin(kMaximumCandidates, samples.size()));
    if (samples.size() <= kMaximumCandidates) {
        candidates = samples;
    } else {
        for (int i = 0; i < kMaximumCandidates; ++i) {
            const qsizetype position = (qsizetype(i) * (samples.size() - 1))
                / (kMaximumCandidates - 1);
            candidates.push_back(samples[int(position)]);
        }
    }
    ...
    double bestAreaSquared = -1.0;
    bool ambiguous = false;
    PlaneHypothesis bestHypothesis;

    const auto samePlane = [](const PlaneHypothesis &left, const PlaneHypothesis &right) {
        const double normalDot = double(QVector3D::dotProduct(left.normal, right.normal));
        if (std::abs(normalDot) < 0.995) return false;
        const double alignedD = normalDot >= 0.0 ? right.d : -right.d;
        return std::abs(left.d - alignedD) <= kPlaneToleranceMm;
    };

    for (int first = 0; first < candidates.size() - 2; ++first) {
        for (int second = first + 1; second < candidates.size() - 1; ++second) {
            const QVector3D p0 = toVector(points[candidates[first]]);
            const QVector3D p1 = toVector(points[candidates[second]]);
            const QVector3D axis = p1 - p0;
            if (axis.lengthSquared() <= 1.0e-12f) continue;
            for (int third = second + 1; third < candidates.size(); ++third) {
                const QVector3D p2 = toVector(points[candidates[third]]);
                const QVector3D cross = QVector3D::crossProduct(axis, p2 - p0);
                const double areaSquared = double(cross.lengthSquared());
                if (areaSquared <= 1.0e-12) continue;

                PlaneHypothesis hypothesis;
                hypothesis.normal = cross.normalized();
                hypothesis.d = -double(QVector3D::dotProduct(hypothesis.normal, p0));
                hypothesis.seeds = {
                    int(sourceIndices[candidates[first]]),
                    int(sourceIndices[candidates[second]]),
                    int(sourceIndices[candidates[third]])};
                hypothesis.areaSquared = areaSquared;
                for (const int sampleIndex : samples) {
                    const QVector3D samplePoint = toVector(points[sampleIndex]);
                    const double distance = std::abs(
                        double(QVector3D::dotProduct(hypothesis.normal, samplePoint))
                        + hypothesis.d);
                    if (distance <= kPlaneToleranceMm) {
                        ++hypothesis.sampleInlierCount;
                    }
                }
                const int inlierCount = hypothesis.sampleInlierCount;

                const bool better = !found || inlierCount > bestInlierCount
                    || (inlierCount == bestInlierCount && areaSquared > bestAreaSquared);
                if (better) {
                    found = true;
                ...
                } else if (found && inlierCount == bestInlierCount
                           && !samePlane(hypothesis, bestHypothesis)) {
                    ambiguous = true;
                }
            }
        }
    ...
    if (!found) {
        return fail(error, QStringLiteral("automatic plane selection found no valid plane with enough inliers"));
    }

    PlaneSelection verifiedSelection;
    if (!buildPlaneSelection(points, sourceIndices, bestHypothesis.seeds,
                             minimumPlaneInliers, &verifiedSelection, error)) {
        return false;
    }
    if (ambiguous) {
        return fail(error, QStringLiteral("automatic plane selection is ambiguous"));
    }
                           ...
                           QString *error)
{
    ...
    *info = {};
    ...
    if (!parseCreatedAt(root, info, error)) return false;
    ...
    if (!info->warning.isEmpty()) warnings.push_back(info->warning);
    if (!info->layoutWarning.isEmpty()) warnings.push_back(info->layoutWarning);
    info->warning = warnings.join(QStringLiteral("; "));
    info->valid = true;
    return true;
}
                                          ...
                                          QString *error)
{
    ...
    // Use the same cached reader as pointcloudview's multi-source loading
    // path. The first read keeps the shared ASCII worker implementation; later
    // runs reuse the validated cache under the application data directory.
    const pcv::detail::io::CachedCloudResult cachedPly =
        pcv::detail::io::readPlyCached(resolvedPointCloud);
    const auto &plyPoints = cachedPly.points;
    if (!cachedPly.ok) {
        return failedResult(QString::fromLatin1(kErrorInputUnsupported), cachedPly.error, error);
    }
    const qsizetype declaredPointCount = plyPoints.size();
    ...
    const pointcloud::RobotCloudResult cloud = pointcloud::transformLineScanToRobotBase(
        plyPoints, calibration, scan.robotPoseStart, scan.robotPoseEnd, transformOptions);
    if (!cloud.ok) {
    ...
    PlaneSelection selection;
    const bool explicitPlaneSeeds = !seedIndices.isEmpty();
    if (explicitPlaneSeeds && !validateSeedIndices(seedIndices, &detail)) {
        return failedResult(QString::fromLatin1(kErrorContract), detail, error);
    }
    const bool planeSelected = explicitPlaneSeeds
        ? buildPlaneSelection(cloud.points, cloud.sourceIndices, seedIndices,
                              options.minimumPlaneInliers, &selection, &detail)
        : buildAutomaticPlaneSelection(cloud.points, cloud.sourceIndices,
                                       options.minimumPlaneInliers, &selection, &detail);
    if (!planeSelected) {
    ...
    result.declaredPointCount = declaredPointCount;
