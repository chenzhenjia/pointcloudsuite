...
- ICP 前在目标主平面的两个切向方向执行有界结构覆盖粗搜索：默认半径 10 mm、粗步长 1 mm、精化步长 0.25 mm；双向 3 mm 覆盖率增益低于 0.05 时不得应用。该修正计入总 `10 mm` 安全范围，粗搜索不得绕过限值。
- 退化配准增加结构点双向覆盖验收：源到目标和目标到源的 3 mm 覆盖率取较小值，低于 0.65 时拒绝；该阈值和点数门槛写入报告。
