...
- `pointcloudprocessor.h/.cpp`：ASCII PLY 读取、无效点过滤、调用手眼转换模块生成机器人基坐标全分辨率点云、任意数量相邻帧 Point-to-Plane ICP、修正范围检查和诊断数据。
...
→ 根据 ICP 后真实投影重叠区确定接缝
→ 双侧有效时羽化融合，否则完整保留来源点
→ binary little-endian 正式点云、预览点云和 stitching_report.json
```
...
- Fitness/RMSE 作为报告指标，不增加参考脚本之外的固定质量硬门槛。
...
- 输出 PLY 为 binary little-endian，仅包含 float `x/y/z`。
