...
PointCloudSuite 面向机器人/工业视觉点云数据的查看、平面提取、障碍检测和二维图像导出。v0.1 的目标是建立一个可运行、可验证、可扩展的点云处理基线：输入 PLY 点云，经过真实点选择和算法处理后，输出平面结果、边缘结果及可追溯的运行文件。
...
- `tests/`：PLY 读取、缓存、滤波、点云处理器和障碍检测测试。

主程序的 `m_points` 是后续处理唯一输入。新点云发布时必须先替换画布缓存，再清除旧平面、障碍、边缘和选点状态。
...
7. 检查障碍告警；确认后选择 X/Y 轴点，生成右手工件坐标系。
...
三点计算初始平面，RANSAC 抵抗离群点，PCA/最小二乘完成精拟合，再按距离阈值分类并做 UV 连通域过滤。障碍默认最小偏离 `1.0 mm`、最小点数 `30`、最小面积 `4.0 mm²`。障碍属于非阻断告警，仍允许继续边缘和图像处理。
...
v0.1 已形成从 PLY 输入、OpenGL 查看、真实点选、平面/障碍/边缘处理到 PNG/JSON/PLY 输出的完整点云处理基线，为后续算法增强、机器人接口和产品化追溯提供稳定文档契约。

## 11. 本版本变更记录

- 建立 PointCloudSuite 主程序与共享模块的产品边界和代码说明。
- 固化 PLY 读取、后台解析、平面/障碍/边缘算法及默认参数。
