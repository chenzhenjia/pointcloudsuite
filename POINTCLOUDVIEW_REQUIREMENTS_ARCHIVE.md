...
障碍物检测必须在用户点击“确定平面”、完成全量平面分类后自动执行，不再等待用户确认候选平面或手动点击检测按钮。输入为当前画布缓存、当前候选平面索引和归一化平面模型。算法将法向统一指向正 Z，但候选判定使用点到基准平面的绝对距离，正侧和负侧都必须检测。这样既覆盖世界坐标中沿正 Z 的凸起，也覆盖 2.5D 相机坐标中因“越靠近相机、深度 Z 越小”而表现为负向距离的障碍。候选点的 UV 投影必须位于平面测量范围内，允许一个栅格边距。

全量平面分类已有的 UV 邻域连通遍历同时承担遮挡检查，不允许再次遍历完整点云。种子点所在区域为主连通面；其他区域同时满足至少 30 个点且点数达到主连通面 0.5% 时，记为显著分离面。显著连通面数量大于 1 表示平面可能被障碍物遮挡，即使高度偏离检测没有形成有效区域，也必须作为工件表面异常告警、将分离区域标红并停止候选确认和后续处理。细小孤立噪声不计入显著连通面。
...
`detectObstacles()` 输出总候选点数、正负侧候选点数、平面显著连通面数量、有效障碍点索引、栅格尺寸和每个区域的方向、点数、面积、中心、平均绝对偏离、最大绝对偏离及三维包围盒。有效障碍点在画布中以红色显示，优先级高于灰色平面和黄色边缘。检测到任一高度异常区域或多个显著连通面时，必须清除已有边缘和 2D 图像结果，禁用候选平面确认并拦截边缘分割、图像提取和边缘选择，同时在状态栏、障碍检测页和警告对话框提示处理已停止。用户明确清除检测结果或发布新的画布缓存后才允许继续处理。障碍检测页的按钮保留为参数调整后的手动重新检测入口。
...
高度阈值、基准平面范围过滤和显著分离平面识别。机器人配准仍以 MSVC 编译、
...
13. 点击“确定平面”后自动执行障碍检测；连续凸起或多个显著连通面显示为红色并弹出提醒，候选确认、边缘和图像处理被阻断；降低凸起高度、恢复单一连通面或移到平面范围外后不应误报。
...
当前阶段依据已确定基准平面的双侧绝对距离、二维连通性、点数、面积和显著分离平面进行规则检测。多个显著连通面属于遮挡证据而非障碍物类别识别结果：开槽、孔洞、装夹结构或两个本来就分离的共面工件也可能触发。没有 CAD、标准件模板或历史无障碍扫描时，算法无法自动区分工件设计中的正常凸台与临时障碍物；因此发现异常后停止下游处理并要求人工复核，不允许简单删除障碍点后假定被遮挡表面完整。
...
```text
727772d Detect two-sided surface obstacles and stop downstream processing
a8991c7 Restore folder preview after obstacle integration
8c13cd5 Add first-stage surface obstacle detection
e5239e4 Remove obsolete test target and sources
