                                           ...
                                           const SeamFusionOptions &options)
{
    ...
    if (options.isCancelled && options.isCancelled()) { result.cancelled = true; return result; }
    ...
    for (int i = 0; i + 1 < centers.size(); ++i) {
        ...
        for (int k = 0; k < merge->points.size(); ++k) { const int id = merge->cloudIds[k]; if (id != i && id != i + 1) continue; const float value = QVector3D::dotProduct(pv(merge->points[k]), s.normal); if (id == i) { s.aMin = qMin(s.aMin, value); s.aMax = qMax(s.aMax, value); } else { s.bMin = qMin(s.bMin, value); s.bMax = qMax(s.bMax, value); } }
    ...
    for (int k = 0; k < merge->points.size(); ++k) { const int id = merge->cloudIds[k]; const QVector3D p = pv(merge->points[k]); if (id > 0 && seams[id - 1].valid && std::abs(QVector3D::dotProduct(p, seams[id - 1].normal) - seams[id - 1].offset) < options.halfWidth) right[id - 1].push_back(k); if (id + 1 < centers.size() && seams[id].valid && std::abs(QVector3D::dotProduct(p, seams[id].normal) - seams[id].offset) < options.halfWidth) left[id].push_back(k); }
    QVector<bool> usable(seams.size(), false); for (int i = 0; i < seams.size(); ++i) usable[i] = seams[i].valid && !left[i].isEmpty() && !right[i].isEmpty();
    QVector<bool> keep(merge->points.size(), true); for (int k = 0; k < merge->points.size(); ++k) { const int id = merge->cloudIds[k]; const QVector3D p = pv(merge->points[k]); if (id > 0 && usable[id - 1]) keep[k] = keep[k] && QVector3D::dotProduct(p, seams[id - 1].normal) - seams[id - 1].offset >= options.halfWidth; if (id + 1 < centers.size() && usable[id]) keep[k] = keep[k] && QVector3D::dotProduct(p, seams[id].normal) - seams[id].offset <= -options.halfWidth; }
